...
class AddOperator extends React.PureComponent <AddOperatorProps, AddOperatorState> {
    ...
    handleChangeColumnSources(columnIndex: number) {
      let sources: number[] = [];
      if (this.state.selectedOperator.sources && this.state.selectedOperator.sources.length>0) {
        sources = this.state.selectedOperator.sources;
      }
      sources.push(columnIndex);
      this.setState({selectedOperator: {...this.state.selectedOperator, sources: sources}});
    }
    ...
    handleChangeParameters(event: React.ChangeEvent<HTMLInputElement>, index: number) {
      ...
      if (this.state.selectedOperator.parameters) {
        ...
        this.setState({selectedOperator: {...this.state.selectedOperator, parameters: updatedParameters}});
      }
    };
            ...
            <div>
              <div>
                ...
                <form noValidate>
                  ...
                  {
                    this.state.selectedOperator.operator && this.state.selectedOperator.operator.columns > 1
                    ?
                    <>
                    {
                      Array.from(Array(this.state.selectedOperator.operator.columns).keys()).map((element: number, index: number) => (
                        <div key={element}>
                        <InputLabel key={'input' + element} htmlFor="max-width">Column {element+1} </InputLabel>
                        <Select key={'slect' + element}
                          autoFocus
                          value={this.state.selectedOperator.sources ? this.state.selectedOperator.sources[index] : 0}
                          onChange={e => {
                            this.handleChangeColumnSources(e.target.value as number);
                          }}
                        >
                          {
                    ...
                    {
                      this.state.selectedOperator.parameters && this.state.selectedOperator.parameters.length > 0 &&
                      <>
                      {
                      this.state.selectedOperator.parameters.map((parameter: Parameter, idx: number) => (
                        <div key={parameter.index}>
                        <InputLabel key={'input' + parameter.index} htmlFor="max-width">Parameter {parameter.index+1} </InputLabel>
                        <TextField
                          id="outlined-basic"
                          size="small"
                          value={this.state.selectedOperator.parameters ? this.state.selectedOperator.parameters[idx].value : ''}
                          onChange={(event:React.ChangeEvent<HTMLInputElement>)  => this.handleChangeParameters(event, idx)}
                          label={parameter.label ? parameter.label : parameter.name}
                          variant="outlined"
                        />
                        </div>
                      ))
                  ...
                  <FormControlLabel
                    ...
                    {
                      this.state.selectedOperator.operator && this.state.selectedOperator.operator.columns > 1 && !this.state.selectedOperator.checked &&
                      <>
