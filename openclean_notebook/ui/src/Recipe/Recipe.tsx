...
class Recipe extends React.PureComponent<RecipeProps, RecipeStates> {
  ...
  isCommited() {
    const list = this.props.operatorProvenance.map(
      operator => operator.isCommitted
    );
  ...
  render() {
    ...
    // option is shown.
    const lastVersion = this.props.operatorProvenance[
      this.props.operatorProvenance.length - 1
    ].id;
    ...
    return (
      <div style={{flex: 'none', width: 300, marginRight: 0}}>
        ...
        >
          ...
          >
            <Button
              ...
              startIcon={
                <PlayArrow fontSize="small" style={{marginRight: '-3px'}} />
              }
        ...
        >
          ...
          <Divider
            light={true}
            style={{color: 'gray', backgroundColor: 'lightgray'}}
          />
          <TabPanel value={this.state.tabValue} index={0}>
            ...
            >
              {this.props.operatorProvenance.length > 0 &&
                this.props.operatorProvenance.map(operator => (
                  <>
                    <div
                      key={operator.id}
                      ...
                      style={{
                        background:
                          operator.id !== versionShown ? '#fff' : '#f5f4fa',
                      }}
                    >
                      <div
                      ...
                      >
                        <div>
                          <span className={'badge badge-pill'}>
                            {(operator.op.optype === 'load' ||
                              operator.op.optype === 'sample') &&
                              'Load'}
                            {operator.op.name}
                          </span>
                          {!operator.isCommitted &&
                            operator.id !== lastVersion && (
                              <button
                                type="button"
                                title="Rollback all changes"
                                className="btn btn-link"
                                onClick={() =>
                                  this.props.onRollback(parseInt(operator.id))
                                }
                        ...
                        <div style={{color: '#5e5e5e'}}>
                          ...
                          >
                            {operator.op.name && (
                              <li>
                                <b>Operator</b>: {operator.op.name}
                              </li>
                            )}
                            {operator.op.optype && (
                              <li>
                                <b>Type</b>: {operator.op.optype}{' '}
                              </li>
                            )}
                            {operator.op.columns && (
                              <li>
                                <b>Column</b>:{' '}
                                {operator.op.columns && operator.op.columns[0]}
                              </li>
                            )}
                          </ul>
                        </div>
                      ...
                      <div
                        ...
                        onClick={() =>
                          this.props.fetchData(parseInt(operator.id), false)
                        }
                      >
              ...
              <Collapse in={this.state.expanded} timeout="auto" unmountOnExit>
                <Card
                  ...
                  style={{backgroundColor: '#dcdcdc', marginRight: 6}}
                >
                  <CardHeader title="Add a new operator" />
                  <CardContent>
                    <AddOperator
                      result={this.props.result}
                      handleDialogExecution={(
