...
def test_dataset_update(persistent_engine, dataset):
    ...
    persistent_engine.apply('my_dataset').update('B', 1)
    persistent_engine.apply('my_dataset').update('C', 2)
    # Get version history.
    snapshots = persistent_engine.history('my_dataset')
    assert len(snapshots) == 3
    # Version 1
    df = persistent_engine.checkout('my_dataset', version=snapshots[0].version)
    assert list(df['A']) == [1, 3]
    assert list(df['B']) == [2, 4]
    assert list(df['C']) == [3, 5]
    # Version 2
    df = persistent_engine.checkout('my_dataset', version=snapshots[1].version)
    assert list(df['A']) == [1, 3]
    assert list(df['B']) == [1, 1]
    assert list(df['C']) == [3, 5]
    # Version 3
    df = persistent_engine.checkout('my_dataset', version=snapshots[2].version)
    assert list(df['A']) == [1, 3]
    assert list(df['B']) == [1, 1]
